  ...
  for(Int_t trg=0; trg<4; trg++)
    {
      ...
      mh3DelPhiInclWt[trg]->Sumw2();
      mh3DelPhiPhotLSWt[trg]->Sumw2();
      mh3DelPhiPhotUSWt[trg]->Sumw2();
      mh2nTracksZdcx[trg]     = new TH2F(Form("mh2nTracksZdcx_%i",trg),"",1000,0,20000,1000,0,1000);
      mh2nTracksBbcx[trg]     = new TH2F(Form("mh2nTracksBbcx_%i",trg),"",1000,0,40000,1000,0,1000);
      mh2nETracksZdcx[trg]    = new TH2F(Form("mh2nETracksZdcx_%i",trg),"",1000,0,20000,1000,0,1000);
      mh2nETracksBbcx[trg]    = new TH2F(Form("mh2nETracksBbcx_%i",trg),"",1000,0,40000,1000,0,1000);
   ...
   for(Int_t trg=0; trg<4; trg++)
     {
       ...
       mh2DelPhiIncl[trg]      -> Write();
       mh2DelPhiPhotLS[trg]    -> Write();
       mh2DelPhiPhotUS[trg]    -> Write();
       ...
       // Pileup Histos
       mh2nTracksZdcx[trg]     -> Write();
       mh2nTracksBbcx[trg]     -> Write();
       mh2nETracksZdcx[trg]    -> Write();
       mh2nETracksBbcx[trg]    -> Write();
...
void StNpeRead::zFillHists (Int_t bTrg,StDmesonEvent * mNpeEvent ,Double_t ps ) // Zach Analysis Loops, only for HT triggers to match Dunkleberger Analysis (comparison)
{
  ...
  zFill_Pileup   (bTrg, mNpeEvent, ps);
}
	 ...
	 pass_cut_Match_EMC_Dz(trk) && pass_cut_Match_EMC_Dphi(trk) && pass_cut_poe(trk) && pass_cut_EMC(trk)) // Is track an electron and the ADC0 big enough to be the trigger?
	{
	  ...
	  for(Int_t ih = 0; ih < mNpeEvent->nTracks(); ih++) // Want to loop over all tracks looking for hads. Not going to double count, since there's only 1 NPE-e/evt on average (in events with NPE, which are rare)
	      ...
	      if(trk != htrk && pass_cut_hTrack(htrk) && !pass_cut_nsigmaE(htrk)) // Is this track a hadron and not the same track or e-?
		{
		  ...
		  mh2DelPhiIncl[trg] -> Fill(dPhi,epT);
	  ...
	  for(Int_t hC=0;hC<4;hC++) //  Study pileup as function of hpT cut
	      ...
	      for(Int_t ih = 0; ih < mNpeEvent->nTracks(); ih++) 
		{
	      ...
	      if(hpTCut > 0.45 && hpTCut < 0.55) // fill 2D for quick analysis of most likely cut
		mh2nETracksZdcx[trg]->Fill(mNpeEvent->ZDCx(),pileupCounter);
...
void StNpeRead::zFill_Pileup (Int_t trg,StDmesonEvent * mNpeEvent ,Double_t ps ) // Fill histograms for events that are non-paired electrons
{
  ...
  mh2nTracksZdcx[trg] -> Fill(zdcx, numTracks);
  mh2nTracksBbcx[trg] -> Fill(bbcx, numTracks);
